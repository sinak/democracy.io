 ...
 * - Non voting delegations e.g. Washington DC, American Samoa, Guam
 *
 */
/**
...
class CongressLegislatorSearch {
  constructor() {
    this.legislators = {
      /** @type {Object<string, CongressLegislator.Legislator[]>} */
      sortedByStates: {},
   ...
   * @returns {CongressLegislator.Legislator | undefined}
   */
  ...
  validDistrict(state, district) {
    ...
    let validState = state in this.legislators;

    return validators.any(v => v === true);
  }

  /**
   * Loads a list of legislators. Replaces any existing legislators.
   * @param {CongressLegislator.Legislator[]} legislatorsList
   * @returns {void}
   */
