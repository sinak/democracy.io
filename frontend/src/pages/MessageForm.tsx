...
import type { TopicOption, CountyData, Legislator, LegislatorFormElements } from '../types';
...
export function MessageForm() {
  ...
  // Form data
  const [formData, setFormData] = useState<Record<string, string>>({ prefix: 'Ms.' });
  ...
  function applyFormFields(formElems: LegislatorFormElements[]) {
    ...
    const filteredElems = formElems.filter((lfe) => bioguideIdsBySelection[lfe.bioguideId]);
  ...
  if (submitted) {
    ...
    if (countyData.options && !formData.county?.trim()) invalidFields.push('County');
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const messages = localLegislators.map((leg) =>
      makeMessage(leg, formData, formData.phone, topicOptions, canonicalAddress)
    );
  ...
  return (
    <div className="row">
      <div className="write-message whitebox col-md-10 ng-enter">
        <button className="btn-sm btn-warning back-button" onClick={() => navigate('/location')}>
          Go back
        </button>
        ...
        {!loading && !sending ? (
          <form className="whitebox-container" onSubmit={handleSubmit} noValidate>
          <div className="row">
                ...
                <div className="panel panel-compact" id="prefixNote">
                  <div className="panel-body">
                    ...
                    <span className="hidden-sm">
                      EFF believes the options provided are limiting and we are looking into
                      alternatives.
                    </span>
                  </div>
                </div>
            ...
            <div className="col-sm-6">
              {Object.values(topicOptions).map((topic) => (
                <div className="form-group" key={topic.bioguideId}>
                  <label htmlFor={`topic-${topic.bioguideId}`}>{topic.name}'s Topic</label>
                  <select
                    className="form-control"
                    id={`topic-${topic.bioguideId}`}
                    value={topic.selected}
                    onChange={(e) => updateTopicSelection(topic.bioguideId, e.target.value)}
                    required
            ...
            {topicFocus && (
              <div className="col-sm-6 hidden-xs form-note ng-hide-remove">
                <div className="panel panel-compact">
                  <div className="panel-body">
                    These topic fields are required by Members of Congress to submit messages via
                    their contact forms.
                  </div>
                </div>
              </div>
        ...
        {submitted && invalidFields.length > 0 && (
          <div className="alert alert-warning" id="messageFormValidation" role="alert">
            <p>
              <strong>Please fill in the following required fields to submit your message:</strong>
            </p>
            <ul>
              {invalidFields.map((f) => (
                <li key={f}>{f}</li>
